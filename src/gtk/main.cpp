...
namespace {
    ...
    void set_status_action(const std::string& text) {
        set_status(g_app.lbl_action_status, text);
    }
    ...
    void refresh_device_list() {
        ...
        try {
            ...
            if (!known.empty()) {
                for (auto& [fingerprint, name_value] : known.items()) {
                    g_app.paired_devices.push_back({fingerprint, name_value.is_string() ? name_value.get<std::string>() : "Unknown Device"});
                }
            }
        } catch (...) {}
        ...
        auto create_row = [](const std::string& name, const std::string& fp, const std::string& ip, uint16_t port, bool paired, bool online) {
            ...
            const char* icon_name = paired ? (online ? "network-cellular-connected-symbolic" : "network-cellular-offline-symbolic") : "dialog-information-symbolic";
        ...
        for (const auto& paired : g_app.paired_devices) {
            bool online = std::find(g_app.connected_fps.begin(), g_app.connected_fps.end(), paired.first) != g_app.connected_fps.end();
        ...
        for (const auto& disc : g_app.discovered_devices) {
            if (disc.fingerprint == my_fp) continue;
            bool is_paired = false;
            for (const auto& p : g_app.paired_devices) {
                if (p.first == disc.fingerprint) { is_paired = true; break; }
            }
            if (is_paired) continue; // already added above
        ...
        for (const auto& req : g_app.pending_pairing_requests) {
            bool is_paired = false;
            for (const auto& p : g_app.paired_devices) {
                if (p.first == req.fingerprint) { is_paired = true; break; }
            }
            if (is_paired) continue; // ignore if they successfully paired

            bool already_shown = false;
            for (const auto& disc : g_app.discovered_devices) {
                if (disc.fingerprint == req.fingerprint) { already_shown = true; break; }
            }
            if (already_shown) continue; // ignore if mDNS already populated it
    ...
    void update_right_pane() {
        ...
        bool is_paired = false;
        for (const auto& p : g_app.paired_devices) {
            if (p.first == g_app.selected_device_fp) is_paired = true;
        }
        ...
        if (is_paired) {
            ...
            bool online = std::find(g_app.connected_fps.begin(), g_app.connected_fps.end(), g_app.selected_device_fp) != g_app.connected_fps.end();
            ...
            set_status(g_app.lbl_unpaired_ip, ("Found at " + g_app.selected_device_ip + ":" + std::to_string(g_app.selected_device_port)));
        }
    }
    ...
    void on_pair_click(GtkWidget*, gpointer) {
        if (g_app.selected_device_ip.empty()) return;
    ...
    void on_accept_click(GtkWidget*, gpointer) {
        if (g_app.selected_device_fp.empty()) return;
        nlohmann::json j;
    ...
    void on_choose_file(GtkWidget*, gpointer) {
        GtkWidget* dialog = gtk_file_chooser_dialog_new(
            "Send File", GTK_WINDOW(g_app.window), GTK_FILE_CHOOSER_ACTION_OPEN,
            "_Cancel", GTK_RESPONSE_CANCEL, "_Send", GTK_RESPONSE_ACCEPT, nullptr);
    ...
    void on_pull_clipboard(GtkWidget*, gpointer) {
        nlohmann::json j; j["command"] = "clipboard_get";
        send_async_daemon_message(j);
    ...
    void on_push_clipboard(GtkWidget*, gpointer) {
        ...
        // Actually, if the user hits "Push", we read local Wayland, then send it via daemon? No, the Daemon already pushes wayland changes!
        // Actually, let's keep the button for explicit "Sync Now" if they disabled auto-sync.
    }
    ...
    void handle_daemon_event(const nlohmann::json& event) {
        ...
        if (command == "state_snapshot") { apply_state_snapshot(event); return; }
        ...
        if (command == "client_disconnected") {
            std::string fp = event.value("fingerprint", "");
            g_app.connected_fps.erase(std::remove(g_app.connected_fps.begin(), g_app.connected_fps.end(), fp), g_app.connected_fps.end());
            refresh_device_list();
            update_right_pane();
            return;
        }
        if (command == "pair_request_received" || command == "untrusted_client_connected") {
            ...
            for (const auto& dev : g_app.pending_pairing_requests) {
                if (dev.fingerprint == req.fingerprint) { exists = true; break; }
            }
            if (!exists) {
                g_app.pending_pairing_requests.push_back(req);
                refresh_device_list();
        ...
        if (command == "pair_accepted") {
            ...
            std::string fp = event.value("fingerprint", "");
            g_app.pending_pairing_requests.erase(
                std::remove_if(g_app.pending_pairing_requests.begin(), g_app.pending_pairing_requests.end(), [&](const tether::DiscoveredDevice& d){ return d.fingerprint == fp; }),
                g_app.pending_pairing_requests.end()
            );
    ...
    void stop_event_subscription() {
        if (g_app.event_watch_id != 0) { g_source_remove(g_app.event_watch_id); g_app.event_watch_id = 0; }
        if (g_app.event_channel) { g_io_channel_shutdown(g_app.event_channel, TRUE, nullptr); g_io_channel_unref(g_app.event_channel); g_app.event_channel = nullptr; } 
        else if (g_app.event_fd >= 0) { close(g_app.event_fd); }
        g_app.event_fd = -1;
    }

    gboolean start_event_subscription(gpointer);
    void schedule_event_retry() {
        if (g_app.event_retry_id == 0) g_app.event_retry_id = g_timeout_add_seconds(2, start_event_subscription, nullptr);
    }

    gboolean on_event_channel(GIOChannel* source, GIOCondition condition, gpointer) {
        if (condition & (G_IO_HUP | G_IO_ERR | G_IO_NVAL)) { stop_event_subscription(); schedule_event_retry(); set_status_main("Daemon Offline"); return FALSE; }
        if (condition & G_IO_IN) {
            gchar* line = nullptr; gsize length = 0; GError* error = nullptr;
            while (true) {
                GIOStatus status = g_io_channel_read_line(source, &line, &length, nullptr, &error);
                if (status == G_IO_STATUS_AGAIN) { if (line) g_free(line); break; }
                if (status == G_IO_STATUS_EOF || status == G_IO_STATUS_ERROR) {
                    if (error) { std::cerr << "Stream err: " << error->message << std::endl; g_error_free(error); }
                    if (line) g_free(line); stop_event_subscription(); schedule_event_retry(); set_status_main("Daemon Offline"); return FALSE;
                }
                if (line && length > 0) { try { handle_daemon_event(nlohmann::json::parse(std::string(line, length))); } catch (...) {} }
                if (line) g_free(line);
            }
        }
        return TRUE;
    ...
    gboolean start_event_subscription(gpointer) {
        if (g_app.event_retry_id != 0) { g_source_remove(g_app.event_retry_id); g_app.event_retry_id = 0; }
        stop_event_subscription();
        int fd = socket(AF_UNIX, SOCK_STREAM, 0);
        if (fd < 0) { schedule_event_retry(); return G_SOURCE_REMOVE; }
            ...
            if (!autostart_attempted) {
                ...
                if (pid == 0) {
                    ...
                    if (fork() == 0) {
                        ...
                        execl(daemon_path.c_str(), "tetherd", nullptr); execlp("tetherd", "tetherd", nullptr); exit(1);
                    }
                    exit(0);
                } else if (pid > 0) { int status; waitpid(pid, &status, 0); }
            }
            schedule_event_retry(); set_status_main("Daemon Offline"); return G_SOURCE_REMOVE;
        }
        ...
        g_app.event_watch_id = g_io_add_watch(g_app.event_channel, static_cast<GIOCondition>(G_IO_IN | G_IO_HUP | G_IO_ERR | G_IO_NVAL), on_event_channel, nullptr);
    ...
    void apply_css() {
        ...
        ".muted { color: #8b949e; }"
        "button { border-radius: 8px; padding: 12px 24px; background-color: #21262d; color: #c9d1d9; border: 1px solid #30363d; font-weight: 600; }"
        "button:hover { background-color: #30363d; }"
        "button.suggested-action { background-color: #238636; color: #ffffff; border-color: rgba(240,246,252,0.1); }"
        "button.suggested-action:hover { background-color: #2ea043; }"
        "list { background-color: transparent; }"
        "list row { background-color: transparent; border-bottom: 1px solid #21262d; padding: 4px; transition: all 150ms ease-in-out; }"
    ...
    void activate(GtkApplication* app, gpointer) {
        ...
        g_signal_connect(refresh_btn, "clicked", G_CALLBACK(+[](GtkWidget*, gpointer){ trigger_discovery(); }), nullptr);
...
int main(int argc, char** argv) {
    ...
    stop_event_subscription();
    if (g_app.event_retry_id != 0) g_source_remove(g_app.event_retry_id);
    g_object_unref(app);
    return status;
}
